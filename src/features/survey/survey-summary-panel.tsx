...
export function SurveySummaryPanel({ summary }: { summary: SurveySummaryStats }) {
  const maxBucket = Math.max(1, ...summary.awarenessDistribution.map((b) => b.count));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card className="p-5">
        ...
        <ul className="mt-3 flex flex-col gap-2" aria-label="Awareness distribution">
          {summary.awarenessDistribution.map((bucket) => {
            const pctOfMax = summary.count === 0 ? 0 : (bucket.count / maxBucket) * 100;
            const pctOfTotal =
              summary.count === 0 ? 0 : Math.round((bucket.count / summary.count) * 100);
            return (
              <li key={bucket.level} className="flex items-center gap-3 text-sm">
