...
describe("computeStatusCode", () => {
  ...
  it("digit 3 (vulnerability) only counts regions under elevated alert", () => {
    ...
    expect(
      computeStatusCode([signal({ heatIndexC: 30, vulnerabilityScore: 90 })]).code[2],
    ).toBe("0");
    // high vulnerability AND Red → 3
    expect(
      computeStatusCode([signal({ heatIndexC: 55, vulnerabilityScore: 90 })]).code[2],
    ).toBe("3");
    // moderate vulnerability + Orange → 1
    expect(
      computeStatusCode([signal({ heatIndexC: 42, vulnerabilityScore: 50 })]).code[2],
    ).toBe("1");
  });

  it("digit 4 (recovery) tracks the peak water-scarcity index", () => {
    expect(computeStatusCode([signal({ waterScarcityIndex: 30 })]).code[3]).toBe("0");
    expect(computeStatusCode([signal({ waterScarcityIndex: 45 })]).code[3]).toBe("1");
    expect(computeStatusCode([signal({ waterScarcityIndex: 60 })]).code[3]).toBe("2");
    expect(computeStatusCode([signal({ waterScarcityIndex: 80 })]).code[3]).toBe("3");
  });

  it("overall state is the worst digit", () => {
    const result = computeStatusCode([
      signal({ heatIndexC: 55, vulnerabilityScore: 90, waterScarcityIndex: 80, feedStale: true }),
    ]);
